...
- `src/focus_mcp/datasets.py`: turns a dataset handle into a location (the
  request's own `X-Focus-Datasets`, the static map, or a raw path over
  stdio) and pools one DuckDB connection per location and credential.
