...
- `focus_queries.py`, `focus_spec_loader.py` — load the query library and the
  spec-derived column/attribute definitions.
...
- `scripts/` — the extractors that produce `resources/` from the published
  spec.
- `Dockerfile`, `.github/workflows/docker-publish.yml` — the image build,
...
Before opening a pull request, lint — it must come back clean:
...
focus_mcp_server.py`) and asking it to inspect the sample data.
...
- **Queries cite their source.** Every entry in `resources/queries/` links the
  ...
  divergence.
- **Version claims are counted, not estimated.** The per-version query counts
  in the README are what the server actually loads — the YAML minus the
...
- **Credentials never cross the MCP boundary.** Anything read from the
  ...
  metadata only.
- Commit subjects in the imperative mood; one logical change per pull request.
